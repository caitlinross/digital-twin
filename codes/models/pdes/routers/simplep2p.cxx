 ...
 */
static void sp_set_params(const char* latency_fname, const char* bw_fname, simplep2p_param* params);
 ...
 * latency/bandwidth */
static double sp_get_table_ent(int from_id, int to_id, int is_outgoing, int num_lps, double* table);

/* category lookup */
static category_idles* sp_get_category_idles(char* category, category_idles* idles);
...
static tw_stime simplep2p_packet_event(model_net_request const* req, uint64_t message_offset,
  uint64_t packet_size, tw_stime offset, mn_sched_params const* sched_params,
  void const* remote_event, void const* self_event, tw_lp* sender, int is_last_pckt);
...
static void sp_event(sp_state* ns, tw_bf* b, sp_message* m, tw_lp* lp);
static void sp_rev_event(sp_state* ns, tw_bf* b, sp_message* m, tw_lp* lp);
static void sp_finalize(sp_state* ns, tw_lp* lp);
...
static void handle_msg_ready_rev_event(sp_state* ns, sp_message* m, tw_lp* lp);
static void handle_msg_ready_event(sp_state* ns, sp_message* m, tw_lp* lp);
static void handle_msg_start_rev_event(sp_state* ns, sp_message* m, tw_lp* lp);
static void handle_msg_start_event(sp_state* ns, sp_message* m, tw_lp* lp);
...
{
  int bufn = 128;
  double* vals = reinterpret_cast<double*>(malloc(bufn * sizeof(double)));
  ...
  *nvals_total = 0;
  ...
  while (line != NULL)
  {
    ...
    while (tok != NULL)
    {
      ...
      {
        bufn <<= 1;
        vals = reinterpret_cast<double*>(realloc(vals, bufn * sizeof(double)));
      }
...
static void sp_set_params(const char* latency_fname, const char* bw_fname, simplep2p_param* params)
{
  ...
  assert(fsize_b >= 0);
  fseek(bf, 0, SEEK_SET);
  char* sbuf = reinterpret_cast<char*>(malloc(fsize_s + 1));
  sbuf[fsize_s] = '\0';
  char* bbuf = reinterpret_cast<char*>(malloc(fsize_b + 1));
  bbuf[fsize_b] = '\0';
  ...
  double* latency_tmp = parse_mat(sbuf, &nvals_first_s, &nvals_total_s, is_tri_mat);
  ...
  if (is_tri_mat)
  {
    params->net_latency_ns_table =
      reinterpret_cast<double*>(malloc(2 * params->mat_len * params->mat_len * sizeof(double)));
...
static void sp_init(sp_state* ns, tw_lp* lp)
{
  ...
  ns->send_next_idle =
    reinterpret_cast<tw_stime*>(malloc(ns->params->num_lps * sizeof(ns->send_next_idle)));
  ns->recv_next_idle =
    reinterpret_cast<tw_stime*>(malloc(ns->params->num_lps * sizeof(ns->recv_next_idle)));
...
static void sp_finalize(sp_state* ns, tw_lp* lp)
{
   ...
   * until afterwards) */
  int i;
  for (i = 0; i < CATEGORY_MAX && strlen(ns->idle_times_cat[i].category) > 0; i++)
  {
...
static void handle_msg_ready_event(sp_state* ns, sp_message* m, tw_lp* lp)
{
  ...
  double bw = sp_get_table_ent(
    m->src_mn_rel_id, ns->id, 1, ns->params->num_lps, ns->params->net_bw_mbps_table);
  double latency = sp_get_table_ent(
    m->src_mn_rel_id, ns->id, 1, ns->params->num_lps, ns->params->net_latency_ns_table);

  // printf("\n LP %d outgoing bandwidth with LP %d is %f ", ns->id, m->src_mn_rel_id, bw);
  if (bw <= 0.0 || latency < 0.0)
  {
    fprintf(stderr, "Invalid link from Rel. id %d to LP %llu (rel. id %d)\n", m->src_mn_rel_id,
      LLU(lp->gid), ns->id);
    abort();
  }
  ...
  if (ns->recv_next_idle[m->src_mn_rel_id] > tw_now(lp))
    recv_queue_time += ns->recv_next_idle[m->src_mn_rel_id] - tw_now(lp);
  ...
  stat = model_net_find_stats(m->category, ns->sp_stats_array);
  category_idles* idles = sp_get_category_idles(m->category, ns->idle_times_cat);
  stat->recv_count++;
  stat->recv_bytes += m->net_msg_size_bytes;
  ...
  printf("%d: BEFORE all_idles_recv %8.3lf %8.3lf\n", ns->id, idles->recv_prev_idle_all,
    idles->recv_next_idle_all);
#endif
  ...
  if (tw_now(lp) > idles->recv_next_idle_all)
  {
    ...
    stat->recv_time += idles->recv_next_idle_all - idles->recv_prev_idle_all;
...
#if SIMPLEP2P_DEBUG
  printf("%d: AFTER  all_idles_recv %8.3lf %8.3lf", ns->id, idles->recv_prev_idle_all,
    idles->recv_next_idle_all);
  ...
  if (m->event_size_bytes)
  {
    ...
    if (m->is_pull)
    {
      struct codes_mctx mc_dst = codes_mctx_set_global_direct(m->src_mn_lp);
      struct codes_mctx mc_src = codes_mctx_set_global_direct(lp->gid);
      int net_id = model_net_get_id(LP_METHOD_NM);
      m->event_rc = model_net_event_mctx(net_id, &mc_src, &mc_dst, m->category, m->src_gid,
        m->pull_size, recv_queue_time, m->event_size_bytes, tmp_ptr, 0, NULL, lp);
    }
    else
    {
...
static void handle_msg_start_event(sp_state* ns, sp_message* m, tw_lp* lp)
{
  ...
  bw = sp_get_table_ent(ns->id, dest_rel_id, 0, ns->params->num_lps, ns->params->net_bw_mbps_table);
  latency =
    sp_get_table_ent(ns->id, dest_rel_id, 0, ns->params->num_lps, ns->params->net_latency_ns_table);

  // printf("\n LP %d incoming bandwidth with LP %d is %f ", ns->id, dest_rel_id, bw);
  if (bw <= 0.0 || latency < 0.0)
  {
    fprintf(stderr, "Invalid link from LP %llu (rel. id %d) to LP %llu (rel. id %d)\n",
  ...
  ns->send_next_idle[dest_rel_id] =
    send_queue_time + tw_now(lp) + rate_to_ns(m->net_msg_size_bytes, bw);
...
#if SIMPLEP2P_DEBUG
  printf("%d: AFTER  all_idles_send %8.3lf %8.3lf", ns->id, idles->send_prev_idle_all,
    idles->send_next_idle_all);
  if (m->local_event_size_bytes > 0)
  {
  ...
  void* m_data;
  e_new = model_net_method_event_new(
    m->dest_mn_lp, send_queue_time + latency, lp, SIMPLEP2P, (void**)&m_new, &m_data);
  ...
  if (m->event_size_bytes)
  {
    memcpy(m_data, model_net_method_get_edata(SIMPLEP2P, m), m->event_size_bytes);
  }
  ...
  if (m->local_event_size_bytes > 0)
  {
    ...
    m_new = reinterpret_cast<sp_message*>(tw_event_data(e_new));

    void* m_loc = (char*)model_net_method_get_edata(SIMPLEP2P, m) + m->event_size_bytes;
    // local_event = (char*)m;
  ...
  void const* remote_event, void const* self_event, tw_lp* sender, int is_last_pckt)
{
  ...
  printf("%lu: final %lu packet sz %d remote sz %d self sz %d is_last_pckt %d latency %lf\n",
    (src_lp - 1) / 2, req->final_dest_lp, packet_size, req->remote_event_size, req->self_event_size,
    is_last_pckt, xfer_to_nic_time + offset);
#endif

  e_new = model_net_method_event_new(
    sender->gid, xfer_to_nic_time + offset, sender, SIMPLEP2P, (void**)&msg, (void**)&tmp_ptr);
...
static double sp_get_table_ent(int from_id, int to_id,
  int is_incoming, /* chooses between incoming and outgoing bandwidths */
  int num_lps, double* table)
{
