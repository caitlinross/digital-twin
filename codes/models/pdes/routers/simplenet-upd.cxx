...
static tw_stime simplenet_packet_event(model_net_request const* req, uint64_t message_offset,
...
static void sn_init(sn_state* ns, tw_lp* lp);
static void sn_event(sn_state* ns, tw_bf* b, sn_message* m, tw_lp* lp);
static void sn_rev_event(sn_state* ns, tw_bf* b, sn_message* m, tw_lp* lp);
static void sn_finalize(sn_state* ns, tw_lp* lp);
...
static void handle_msg_ready_rev_event(sn_state* ns, sn_message* m, tw_lp* lp);
static void handle_msg_ready_event(sn_state* ns, sn_message* m, tw_lp* lp);
static void handle_msg_start_rev_event(sn_state* ns, sn_message* m, tw_lp* lp);
static void handle_msg_start_event(sn_state* ns, sn_message* m, tw_lp* lp);
...
static void handle_msg_ready_event(sn_state* ns, sn_message* m, tw_lp* lp)
{
  ...
  stat->recv_time += rate_to_ns(m->net_msg_size_bytes, ns->params.net_bw_mbps);
  ...
  recv_queue_time += rate_to_ns(m->net_msg_size_bytes, ns->params.net_bw_mbps);
...
static void handle_msg_start_event(sn_state* ns, sn_message* m, tw_lp* lp)
{
  ...
  total_event_size =
    model_net_get_msg_sz(SIMPLENET) + m->event_size_bytes + m->local_event_size_bytes;
  ...
  stat->send_time +=
    (ns->params.net_startup_ns + rate_to_ns(m->net_msg_size_bytes, ns->params.net_bw_mbps));
  ...
  ns->net_send_next_idle =
    send_queue_time + tw_now(lp) + rate_to_ns(m->net_msg_size_bytes, ns->params.net_bw_mbps);

  void* m_data;
  e_new = model_net_method_event_new(
    m->dest_mn_lp, send_queue_time, lp, SIMPLENET, (void**)&m_new, &m_data);
  ...
  if (m->event_size_bytes)
  {
    memcpy(m_data, model_net_method_get_edata(SIMPLENET, m), m->event_size_bytes);
  }
  ...
  model_net_method_idle_event(codes_local_latency(lp) + ns->net_send_next_idle - tw_now(lp), 0, lp);
  ...
  if (m->local_event_size_bytes > 0)
  {
    ...
    m_new = reinterpret_cast<sn_message*>(tw_event_data(e_new));

    void* m_loc = (char*)model_net_method_get_edata(SIMPLENET, m) + m->event_size_bytes;
  ...
  void const* remote_event, void const* self_event, tw_lp* sender, int is_last_pckt)
{
  ...
  e_new = model_net_method_event_new(
    sender->gid, xfer_to_nic_time + offset, sender, SIMPLENET, (void**)&msg, (void**)&tmp_ptr);
