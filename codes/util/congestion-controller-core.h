...
extern "C"
{
  ...
  typedef enum cc_event_t
  {
    ...
    CC_SIGNAL_ABATE,
    CC_BANDWIDTH_CHECK,
    CC_SIM_ACK // A 'simulated' ack message sent from receiving terminal to original to let it know
               // that its packet was ejected
  } cc_event_t;
  ...
  typedef struct congestion_control_message
  {
    ...
    // Dangerous - same LP dynamic RC state -- if this message is to be sent between two LPs, DON'T
    // USE THIS FIELD
    size_t size_abated;
