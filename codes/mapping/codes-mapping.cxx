...
static int cmp_anno(const char* anno_user, const char* anno_config)
{
  return anno_user == NULL ? anno_config == NULL
                           : (anno_config != NULL && !strcmp(anno_user, anno_config));
}
...
static char local_lp_name[MAX_NAME_LENGTH], local_annotation[MAX_NAME_LENGTH];
  ...
  const char* lp_type_name, const char* annotation, int ignore_annos)
{
  ...
  for (int g = 0; g < lpconf.lpgroups_count; g++)
  {
    ...
    if (group_name == NULL || strcmp(lpg->name.ptr, group_name) == 0)
    {
      ...
      if (group_name != NULL)
        break; // we've processed the exact group
    }
  }
...
void codes_mapping_get_lp_id(const char* group_name, const char* lp_type_name,
  const char* annotation, int ignore_anno, int rep_id, int offset, tw_lpid* gid)
{
  // sanity checks
      ...
      for (int l = 0; l < lpg->lptypes_count; l++)
      {
        ...
        if (strcmp(lpt->name.ptr, lp_type_name) == 0)
        {
          ...
          if (ignore_anno || cmp_anno(annotation, lpt->anno.ptr))
          {
            ...
            {
              *gid += (rep_count * (tw_lpid)rep_id) + local_lp_count + (tw_lpid)offset;
              return;
            }
          }
        }
...
ERROR:
  ...
  tw_error(TW_LOC,
    ...
    group_name == NULL ? "<NULL>" : group_name, lp_type_name == NULL ? "<NULL>" : lp_type_name,
    annotation == NULL ? "<NULL>" : annotation, rep_id, offset);
}
...
int codes_mapping_get_lp_relative_id(tw_lpid gid, int group_wise, int annotation_wise)
{
  ...
  codes_mapping_get_lp_info(
    gid, NULL, &group_index, local_lp_name, &lp_type_index, local_annotation, &rep_id, &offset);
...
tw_lpid codes_mapping_get_lpid_from_relative(int relative_id, const char* group_name,
  const char* lp_type_name, const char* annotation, int annotation_wise)
{
  ...
  for (int g = 0; g < lpconf.lpgroups_count; g++)
  {
    ...
    if (group_name == NULL || strcmp(group_name, lpg->name.ptr) == 0)
    {
      ...
      if (relative_id < rel_id_count + lpg->repetitions * local_rel_id_count)
      {
...
{
  int index = lpid - (g_tw_mynode * lps_per_pe_floor) - mini(g_tw_mynode, lps_leftover);
  //   printf("\n global id %d index %d lps_before %d lps_offset %d local index %d ", lpid, index,
  //   lps_before, g_tw_mynode, local_index);
...
void codes_mapping_setup_with_seed_offset(int offset)
{
  ...
  for (grp = 0; grp < lpconf.lpgroups_count; grp++)
  {
    for (lpt = 0; lpt < lpconf.lpgroups[grp].lptypes_count; lpt++)
      lps_per_pe_floor +=
        (lpconf.lpgroups[grp].lptypes[lpt].count * lpconf.lpgroups[grp].repetitions);
  }
  ...
  int mem_factor_conf;
  int rc = configuration_get_value_int(&config, "PARAMS", "pe_mem_factor", NULL, &mem_factor_conf);
...
const char* codes_mapping_get_annotation_by_name(const char* group_name, const char* lp_type_name)
{
  ...
  tw_error(TW_LOC,
    "unable to find annotation using "
    "group \"%s\" and lp_type_name \"%s\"",
    group_name, lp_type_name);
  return NULL;
}
...
const char* codes_mapping_get_annotation_by_lpid(tw_lpid gid)
{
  ...
  codes_mapping_get_lp_info(gid, NULL, &group_index, NULL, &lp_type_index, NULL, &dummy, &dummy);
...
static int get_cid_by_name(char const* name, char const** names, int num_names)
{
...
static char const* get_name_by_cid(int cid, char const** names, int num_names)
{
...
{
  return get_cid_by_name(group_name, lpconf.group_names, lpconf.lpgroups_count);
}
...
int codes_mapping_get_group_cid_by_lpid(tw_lpid id)
{
  ...
  int ignore;
  codes_mapping_get_lp_info(id, group_name, &ignore, NULL, &ignore, NULL, &ignore, &ignore);
...
{
  return get_cid_by_name(lp_type_name, lpconf.lp_names, lpconf.num_uniq_lptypes);
}
...
int codes_mapping_get_lp_cid_by_lpid(tw_lpid id)
{
  ...
  int ignore;
  codes_mapping_get_lp_info(id, NULL, &ignore, lp_name, &ignore, NULL, &ignore, &ignore);
...
int codes_mapping_get_anno_cid_by_name(char const* annotation)
{
  if (annotation == NULL || annotation[0] == '\0')
    ...
    return get_cid_by_name(annotation, lpconf.anno_names, lpconf.num_uniq_annos);
}
...
int codes_mapping_get_anno_cid_by_lpid(tw_lpid id)
{
  ...
  int ignore;
  codes_mapping_get_lp_info(id, NULL, &ignore, NULL, &ignore, anno, &ignore, &ignore);
