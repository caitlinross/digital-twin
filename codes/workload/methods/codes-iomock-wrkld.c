...
{
  return (a->file_id == b->file_id && a->use_uniq_file_ids == b->use_uniq_file_ids &&
          a->is_write == b->is_write && a->num_requests == b->num_requests &&
  ...
  qlist_for_each(ent, &app_params_list)
  {
    ...
    if (app_id == ap->app_id)
    {
      if (params_eq(&ap->params, p))
        ...
        tw_error(TW_LOC, "app %d: rank %d passed in a different config", app_id, rank);
    }
  }
...
static void iomock_workload_get_next(int app_id, int rank, struct codes_workload_op* op)
{
  ...
  struct rank_state* rs = qhash_entry(hash_link, struct rank_state, hash_link);
  ...
  else
  {
    if (p->is_write)
    {
      ...
      op->u.write.offset = (uint64_t)p->request_size * (uint64_t)rs->reqs_processed;
    }
    else
    {
      ...
      op->u.read.offset = (uint64_t)p->request_size * (uint64_t)rs->reqs_processed;
    }
    rs->reqs_processed++;
  }
  return;
