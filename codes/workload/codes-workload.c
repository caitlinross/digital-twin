...
static struct codes_workload_method const* method_array_default[] = { &test_workload_method,
#ifdef USE_DUMPI
...
#endif
  &checkpoint_workload_method, &iomock_workload_method, NULL };
...
static void init_workload_methods(void)
{
  ...
  else
  {
    ...
    int num_default_methods = (sizeof(method_array_default) / sizeof(method_array_default[0]));
    printf("\n Num default methods %d ", num_default_methods);
    method_array =
      realloc(method_array, (num_default_methods + num_user_methods + 1) * sizeof(*method_array));
  ...
  for (int i = 0; method_array[i] != NULL; i++)
  {
    ...
    if (strcmp(m->method_name, type) == 0)
    {
      ...
      if (m->codes_workload_read_config == NULL)
        ...
        r.params = m->codes_workload_read_config(section_name, annotation, num_ranks);
    }
  }
...
int codes_workload_finalize(const char* type, const char* params, int app_id, int rank)
{
  ...
  for (i = 0; method_array[i] != NULL; i++)
  {
    if (strcmp(method_array[i]->method_name, type) == 0)
    {
      return method_array[i]->codes_workload_finalize(params, app_id, rank);
    }
  }
  ...
  double* read_time, double* write_time, int64_t* read_bytes, int64_t* written_bytes)
{
  ...
  for (i = 0; method_array[i] != NULL; i++)
  {
    ...
    // method_array[i]->method_name, type);
    if (strcmp(method_array[i]->method_name, type) == 0)
    {
  ...
  switch (op->op_type)
  {
    ...
    case CODES_WK_BARRIER:
      fprintf(f, "op: app:%d rank:%d type:barrier count:%d root:%d\n", app_id, rank,
        op->u.barrier.count, op->u.barrier.root);
      break;
    case CODES_WK_OPEN:
    case CODES_WK_MPI_OPEN:
    case CODES_WK_MPI_COLL_OPEN:
      if (op->op_type == CODES_WK_OPEN)
        name = "open";
      if (op->op_type == CODES_WK_MPI_OPEN)
        name = "mpi_open";
      if (op->op_type == CODES_WK_MPI_COLL_OPEN)
        name = "mpi_coll_open";
      fprintf(f, "op: app:%d rank:%d type:%s file_id:%llu flag:%d\n", app_id, rank, name,
        LLU(op->u.open.file_id), op->u.open.create_flag);
      break;
    case CODES_WK_CLOSE:
    case CODES_WK_MPI_CLOSE:
      if (op->op_type == CODES_WK_CLOSE)
        name = "close";
      if (op->op_type == CODES_WK_MPI_CLOSE)
        name = "mpi_close";
      fprintf(f, "op: app:%d rank:%d type:%s file_id:%llu\n", app_id, rank, name,
        LLU(op->u.close.file_id));
      break;
    case CODES_WK_WRITE:
    case CODES_WK_MPI_WRITE:
    case CODES_WK_MPI_COLL_WRITE:
      if (op->op_type == CODES_WK_WRITE)
        name = "write";
      if (op->op_type == CODES_WK_MPI_WRITE)
        name = "mpi_write";
      if (op->op_type == CODES_WK_MPI_COLL_WRITE)
        name = "mpi_coll_write";
      fprintf(f,
        "op: app:%d rank:%d type:%s "
    ...
    case CODES_WK_MPI_COLL_READ:
      if (op->op_type == CODES_WK_READ)
        name = "read";
      if (op->op_type == CODES_WK_MPI_READ)
        name = "mpi_read";
      if (op->op_type == CODES_WK_MPI_COLL_READ)
        name = "mpi_coll_read";
      fprintf(f,
        "op: app:%d rank:%d type:%s "
        "file_id:%llu off:%llu size:%llu\n",
        app_id, rank, name, LLU(op->u.read.file_id), LLU(op->u.read.offset), LLU(op->u.read.size));
      break;
    case CODES_WK_SEND:
      fprintf(f,
        ...
        "start:%.5e end:%.5e\n",
        app_id, rank, op->u.send.source_rank, op->u.send.dest_rank, op->u.send.num_bytes,
        op->u.send.data_type, op->u.send.count, op->u.send.tag, op->start_time, op->end_time);
      break;
    case CODES_WK_RECV:
      fprintf(f,
        ...
        "src:%d dst:%d bytes:%" PRIu64 " type:%d count:%d tag:%d "
        "start:%.5e end:%.5e\n",
        app_id, rank, op->u.recv.source_rank, op->u.recv.dest_rank, op->u.recv.num_bytes,
        op->u.recv.data_type, op->u.recv.count, op->u.recv.tag, op->start_time, op->end_time);
      break;
    case CODES_WK_ISEND:
      fprintf(f,
        ...
        "start:%.5e end:%.5e\n",
        app_id, rank, op->u.send.source_rank, op->u.send.dest_rank, op->u.send.req_id,
        op->u.send.num_bytes, op->u.send.data_type, op->u.send.count, op->u.send.tag,
    ...
    case CODES_WK_IRECV:
      fprintf(f,
        ...
        "src:%d dst:%d req_id:%" PRIu32 " bytes:%" PRIu64 " type:%d count:%d tag:%d "
        "start:%.5e end:%.5e\n",
        app_id, rank, op->u.recv.source_rank, op->u.recv.dest_rank, op->u.recv.req_id,
        op->u.recv.num_bytes, op->u.recv.data_type, op->u.recv.count, op->u.recv.tag,
        op->start_time, op->end_time);
      break;
    case CODES_WK_REQ_FREE:
      fprintf(f,
        ...
        " req:%d ",
        app_id, rank, op->u.free.req_id);
      break;
#define PRINT_COL(_type_str)                                                                       \
  fprintf(f,                                                                                       \
    "op: app:%d rank:%d type:%s"                                                                   \
    " bytes:%d, start:%.5e, end:%.5e\n",                                                           \
    app_id, rank, _type_str, op->u.collective.num_bytes, op->start_time, op->end_time)
    case CODES_WK_BCAST:
      PRINT_COL("bcast");
      break;
...
void codes_workload_add_method(struct codes_workload_method const* method)
{
  ...
  if (is_workloads_init)
    tw_error(TW_LOC, "adding a workload method after initialization is forbidden");
  ...
  {
    method_array_cap *= 2;
    method_array = realloc(method_array, method_array_cap * sizeof(*method_array));
    assert(method_array);
  }
