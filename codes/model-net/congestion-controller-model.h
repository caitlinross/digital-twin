 ...
 *  This structure is designed to obey the child sum property. The values stored in a parent
 * represents the sum of the values of its children
 */
...
private:
  ...
  map<unsigned int, unsigned int>
    abated_terminal_child_counter; // maps terminal ID to number of children nodes that it is under
                                   // abatement on
  ...
  map<unsigned int, map<unsigned int, unsigned long long>>
    app_to_terminal_counter; // maps application ID to a map of terminals from said application and
                             // their packet counts on this node and children
...
public:
  ...
  unsigned long long get_packet_count_by_port_vc_from_term(
    int port_no, int vc_no, unsigned int term_id);
  unsigned long long get_packet_count_by_port_vc_from_app(
    int port_no, int vc_no, unsigned int app_id);
  ...
  map<unsigned int, unsigned long long> get_term_count_map_by_port_from_app(
    int port_no, int app_id);
  map<unsigned int, unsigned long long> get_term_count_map_by_port_vc_from_app(
...
typedef struct cc_param
{
  ...
  double single_port_aggressor_usage_threshold; // percentage of current usage belonging to one app
                                                // before its classified as an aggressor
