...
#define X(a, b, c, d) d,
struct model_net_method* method_array[] = { NETWORK_DEF };
#undef X
...
double cn_bandwidth = 20;
tw_stime codes_cn_delay;
int codes_node_eager_limit = 16000;
...
int* model_net_configure(int* id_count)
{
  ...
  *id_count = 0;
  ...
  size_t length;
  int ret =
    configuration_get_multivalue(&config, "PARAMS", "modelnet_order", NULL, &values, &length);
  if (ret != 1)
  {
  ...
  ret = configuration_get_value_double(&config, "PARAMS", "cn_bandwidth", NULL, &cn_bandwidth);
  if (ret && !g_tw_mynode)
  {
    fprintf(stderr,
  ...
  ret = configuration_get_value_int(
    &config, "PARAMS", "node_eager_limit", NULL, &codes_node_eager_limit);
  if (ret && !g_tw_mynode)
  {
    fprintf(stderr,
  ...
  ret =
    configuration_get_value_int(&config, "PARAMS", "codes_noop_bypass", NULL, &codes_noop_bypass);
  if (!ret && !g_tw_mynode)
  {
...
void model_net_write_stats(tw_lpid lpid, struct mn_stats* stat)
{
  ...
  sprintf(data,
    ...
    stat->recv_bytes, stat->recv_time, stat->max_event_size);
...
static model_net_event_return model_net_noop_event(tw_lpid final_dest_lp, int is_pull,
  tw_stime offset, uint64_t message_size, int remote_event_size, void const* remote_event,
  int self_event_size, void const* self_event, tw_lp* sender)
{
  ...
  if (remote_event_size && remote_event != NULL)
  {
     ...
     * to self */
    tw_event* e =
      tw_event_new(is_pull ? sender->gid : final_dest_lp, poffset + offset + sendTime, sender);
...
static model_net_event_return model_net_event_impl_base(int net_id,
  struct codes_mctx const* send_map_ctx, struct codes_mctx const* recv_map_ctx,
  char const* category, tw_lpid final_dest_lp, uint64_t message_size, int is_pull, tw_stime offset,
  int remote_event_size, void const* remote_event, int self_event_size, void const* self_event,
  tw_lp* sender)
{

  if (remote_event_size + self_event_size + sizeof(model_net_wrap_msg) > g_tw_msg_sz)
  {
    tw_error(TW_LOC,
      "Error: model_net trying to transmit an event of size "
      "%d but ROSS is configured for events of size %zd\n",
      remote_event_size + self_event_size + sizeof(model_net_wrap_msg), g_tw_msg_sz);
    return -1;
  }

  tw_lpid src_mn_lp = model_net_find_local_device_mctx(net_id, send_map_ctx, sender->gid);
  tw_lpid dest_mn_lp = model_net_find_local_device_mctx(net_id, recv_map_ctx, final_dest_lp);

  if (src_mn_lp == dest_mn_lp && message_size < (uint64_t)codes_node_eager_limit &&
      !codes_noop_bypass)
  {
    return model_net_noop_event(final_dest_lp, is_pull, offset, message_size, remote_event_size,
  ...
  { // perhaps make jobmap a global set regardless of congestion control
...
model_net_event_return model_net_event(int net_id, char const* category, tw_lpid final_dest_lp,
  ...
  int self_event_size, void const* self_event, tw_lp* sender)
{
  return model_net_event_impl_base(net_id, CODES_MCTX_DEFAULT, CODES_MCTX_DEFAULT, category,
    final_dest_lp, message_size, 0, offset, remote_event_size, remote_event, self_event_size,
    self_event, sender);
}
  ...
  tw_lp* sender)
{
  ...
  return model_net_event_impl_base(net_id, &mc, &mc, category, final_dest_lp, message_size, 0,
    offset, remote_event_size, remote_event, self_event_size, self_event, sender);
}
...
model_net_event_return model_net_event_mctx(int net_id, struct codes_mctx const* send_map_ctx,
  ...
  int self_event_size, void const* self_event, tw_lp* sender)
{
  return model_net_event_impl_base(net_id, send_map_ctx, recv_map_ctx, category, final_dest_lp,
    message_size, 0, offset, remote_event_size, remote_event, self_event_size, self_event, sender);
}

model_net_event_return model_net_pull_event(int net_id, char const* category, tw_lpid final_dest_lp,
  uint64_t message_size, tw_stime offset, int self_event_size, void const* self_event,
  tw_lp* sender)
{
  /* NOTE: for a pull, we are filling the *remote* event - it will be remote
   * from the destination's POV */
  return model_net_event_impl_base(net_id, CODES_MCTX_DEFAULT, CODES_MCTX_DEFAULT, category,
    final_dest_lp, message_size, 1, offset, self_event_size, self_event, 0, NULL, sender);
}
  ...
  int self_event_size, void const* self_event, tw_lp* sender)
{
   ...
   * from the destination's POV */
  struct codes_mctx mc = codes_mctx_set_group_modulo(annotation, false);
  return model_net_event_impl_base(net_id, &mc, &mc, category, final_dest_lp, message_size, 1,
  ...
  tw_lp* sender)
{
   ...
   * from the destination's POV */
  return model_net_event_impl_base(net_id, send_map_ctx, recv_map_ctx, category, final_dest_lp,
    message_size, 1, offset, self_event_size, self_event, 0, NULL, sender);
}
...
void model_net_event_collective(int net_id, char const* category, int message_size,
  int remote_event_size, const void* remote_event, tw_lp* sender)
{
  if (net_id < 0 || net_id > MAX_NETS)
  {
    fprintf(stderr, "%s Error: Uninitializied modelnet network, call modelnet_init first\n",
  ...
  return method_array[net_id]->mn_collective_call(
...
const st_model_types* model_net_get_model_stat_type(int net_id)
{
  if (!method_array[net_id]->mn_get_model_stat_types)
    tw_error(
      TW_LOC, "mn_get_model_stat_types not defined for network model with net_id: %d\n", net_id);
...
{
  return codes_mctx_to_lpid(map_ctx, model_net_lp_config_names[net_id], sender_gid);
}
