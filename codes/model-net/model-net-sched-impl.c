...
#include "codes/model-net/model-net-sched-impl.h"
...
static void fcfs_init(const struct model_net_method* method,
  const model_net_sched_cfg_params* params, int is_recv_queue, void** sched);
static void fcfs_destroy(void* sched);
static void fcfs_add(const model_net_request* req, const mn_sched_params* sched_params,
  int remote_event_size, void* remote_event, int local_event_size, void* local_event, void* sched,
...
static int fcfs_next(
  tw_stime* poffset, void* sched, void* rc_event_save, model_net_sched_rc* rc, tw_lp* lp);
static void fcfs_next_rc(
  void* sched, const void* rc_event_save, const model_net_sched_rc* rc, tw_lp* lp);

// ROUND-ROBIN
static void rr_init(const struct model_net_method* method, const model_net_sched_cfg_params* params,
  int is_recv_queue, void** sched);
static void rr_destroy(void* sched);
static void rr_add(const model_net_request* req, const mn_sched_params* sched_params,
  int remote_event_size, void* remote_event, int local_event_size, void* local_event, void* sched,
  model_net_sched_rc* rc, tw_lp* lp);
...
static void rr_next_rc(
  void* sched, const void* rc_event_save, const model_net_sched_rc* rc, tw_lp* lp);
static void prio_init(const struct model_net_method* method,
  const model_net_sched_cfg_params* params, int is_recv_queue, void** sched);
static void prio_destroy(void* sched);
static void prio_add(const model_net_request* req, const mn_sched_params* sched_params,
  int remote_event_size, void* remote_event, int local_event_size, void* local_event, void* sched,
  model_net_sched_rc* rc, tw_lp* lp);
...
static const model_net_sched_interface fcfs_tab = { &fcfs_init, &fcfs_destroy, &fcfs_add,
  &fcfs_add_rc, &fcfs_next, &fcfs_next_rc };
static const model_net_sched_interface rr_tab = { &rr_init, &rr_destroy, &rr_add, &rr_add_rc,
  &rr_next, &rr_next_rc };
static const model_net_sched_interface prio_tab = { &prio_init, &prio_destroy, &prio_add,
  &prio_add_rc, &prio_next, &prio_next_rc };

#define X(a, b, c) c,
const model_net_sched_interface* sched_interfaces[] = { SCHEDULER_TYPES };
#undef X

/// FCFS implementation

void fcfs_init(const struct model_net_method* method, const model_net_sched_cfg_params* params,
  int is_recv_queue, void** sched)
{
...
void fcfs_add(const model_net_request* req, const mn_sched_params* sched_params,
  int remote_event_size, void* remote_event, int local_event_size, void* local_event, void* sched,
  model_net_sched_rc* rc, tw_lp* lp)
{
  ...
  dprintf("%llu (mn):    adding %srequest from %llu to %llu, size %llu, at %lf\n", LLU(lp->gid),
    req->is_pull ? "pull " : "", LLU(req->src_lp), LLU(req->final_dest_lp), LLU(req->msg_size),
    tw_now(lp));
}
  ...
  tw_stime* poffset, void* sched, void* rc_event_save, model_net_sched_rc* rc, tw_lp* lp)
{
  ...
  if (s->is_recv_queue)
  {
    dprintf("%llu (mn):    receiving message of size %llu (of %llu) "
      ...
      LLU(lp->gid), LLU(psize), LLU(q->rem), LLU(q->req.src_lp), LLU(q->req.final_dest_lp),
      tw_now(lp), is_last_packet);
    // note: we overloaded on the dest_mn_lp field - it's the dest of the
    // soruce in the case of a pull
    *poffset = s->method->model_net_method_recv_msg_event(q->req.category, q->req.final_dest_lp,
      q->req.dest_mn_lp, psize, q->req.is_pull, q->req.pull_size, 0.0, q->req.remote_event_size,
  ...
  else
  {
    dprintf("%llu (mn):    issuing packet of size %llu (of %llu) "
      ...
      LLU(lp->gid), LLU(psize), LLU(q->rem), LLU(q->req.src_lp), LLU(q->req.final_dest_lp),
      tw_now(lp), is_last_packet);
    *poffset = s->method->model_net_method_packet_event(&q->req, q->req.msg_size - q->rem, psize,
      0.0, &q->sched_params, q->remote_event, q->local_event, lp, is_last_packet);
  }
  ...
  if (is_last_packet)
  {
    dprintf("last %spkt: %llu (%llu) to %llu, size %llu at %1.5e (pull:%d)\n",
      s->is_recv_queue ? "recv " : "send ", LLU(lp->gid), LLU(q->req.src_lp),
      LLU(q->req.final_dest_lp), LLU(q->req.is_pull ? q->req.pull_size : q->req.msg_size),
      tw_now(lp), q->req.is_pull);
    qlist_pop(&s->reqs);
    s->queue_len--;
    rc->req = q->req;
...
void fcfs_next_rc(void* sched, const void* rc_event_save, const model_net_sched_rc* rc, tw_lp* lp)
{
  mn_sched_queue* s = sched;
  if (rc->rtn == -1)
  {
...
void rr_init(const struct model_net_method* method, const model_net_sched_cfg_params* params,
  int is_recv_queue, void** sched)
{
...
void rr_add(const model_net_request* req, const mn_sched_params* sched_params,
  int remote_event_size, void* remote_event, int local_event_size, void* local_event, void* sched,
  model_net_sched_rc* rc, tw_lp* lp)
{
  fcfs_add(req, sched_params, remote_event_size, remote_event, local_event_size, local_event, sched,
    rc, lp);
}
...
int rr_next(tw_stime* poffset, void* sched, void* rc_event_save, model_net_sched_rc* rc, tw_lp* lp)
{
...
void rr_next_rc(void* sched, const void* rc_event_save, const model_net_sched_rc* rc, tw_lp* lp)
{
...
void prio_init(const struct model_net_method* method, const model_net_sched_cfg_params* params,
  int is_recv_queue, void** sched)
{
  *sched = malloc(sizeof(mn_sched_prio));
  ...
  {
    ss->sub_sched_iface->init(method, params, is_recv_queue, (void**)&ss->sub_scheds[i]);
  }
}
...
void prio_add(const model_net_request* req, const mn_sched_params* sched_params,
  int remote_event_size, void* remote_event, int local_event_size, void* local_event, void* sched,
  model_net_sched_rc* rc, tw_lp* lp)
{
  ...
  {
    tw_error(TW_LOC, "sched for lp %llu: invalid prio (%d vs [%d,%d))", LLU(lp->gid), prio, 0,
      ss->params.num_prios);
  }
  dprintf("%llu (mn):    adding with prio %d\n", LLU(lp->gid), prio);
  ss->sub_sched_iface->add(req, sched_params, remote_event_size, remote_event, local_event_size,
    local_event, ss->sub_scheds[prio], rc, lp);
  rc->prio = prio;
}
  ...
  tw_stime* poffset, void* sched, void* rc_event_save, model_net_sched_rc* rc, tw_lp* lp)
{
  ...
  for (int i = 0; i < ss->params.num_prios; i++)
  {
    ...
    {
      rc->prio = i;
      return ss->sub_sched_iface->next(poffset, ss->sub_scheds[i], rc_event_save, rc, lp);
    }
  }
  rc->prio = -1;
...
void prio_next_rc(void* sched, const void* rc_event_save, const model_net_sched_rc* rc, tw_lp* lp)
{
  if (rc->prio != -1)
  {
    // we called a next somewhere
    mn_sched_prio* ss = sched;
    ss->sub_sched_iface->next_rc(ss->sub_scheds[rc->prio], rc_event_save, rc, lp);
  }
  // else, no-op
}
