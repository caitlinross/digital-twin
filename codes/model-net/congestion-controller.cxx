...
#include <codes/model-net/congestion-controller-model.h>
...
const tw_optdef cc_app_opt[] = { TWOPT_GROUP("Congestion Control"),
  TWOPT_STIME("cc_notif_latency", g_congestion_control_notif_latency,
    "latency for congestion control notifications"),
  TWOPT_END() };
...
Portchan_node::Portchan_node(portchan_node_type pnt, int router_radix, int vcs_per_port)
{
  ...
  switch (type)
  {
    case ROOT:
      ...
      {
        Portchan_node* child =
          new Portchan_node(PORT, router_radix, vcs_per_port); // delete done in deconstructor
    ...
    case PORT:
      ...
      {
        Portchan_node* child =
          new Portchan_node(VC, router_radix, vcs_per_port); // delete done in deconstructor
        children.push_back(child);
      }
      break;
...
unsigned long long Portchan_node::get_packet_count_by_port_from_term(
  int port_no, unsigned int term_id)
{
  if (type != ROOT)
    tw_error(TW_LOC, "Portchan_node : Invalid node type for this action\n");
...
unsigned long long Portchan_node::get_packet_count_by_port_vc_from_term(
  int port_no, int vc_no, unsigned int term_id)
{
  if (type != ROOT)
    tw_error(TW_LOC, "Portchan_node : Invalid node type for this action\n");
...
map<unsigned int, unsigned long long> Portchan_node::get_term_count_map_by_port_vc(
  int port_no, int vc_no)
{
  if (type != ROOT)
    tw_error(TW_LOC, "Portchan_node : Invalid node type for this action\n");
...
map<unsigned int, unsigned long long> Portchan_node::get_app_count_map_by_port_vc(
  int port_no, int vc_no)
{
  if (type != ROOT)
    tw_error(TW_LOC, "Portchan_node : Invalid node type for this action\n");
...
map<unsigned int, unsigned long long> Portchan_node::get_term_count_map_by_port_from_app(
  int port_no, int app_id)
{
  if (type != ROOT)
    tw_error(TW_LOC, "Portchan_node : Invalid node type for this action\n");
...
map<unsigned int, unsigned long long> Portchan_node::get_term_count_map_by_port_vc_from_app(
  int port_no, int vc_no, int app_id)
{
  if (type != ROOT)
    tw_error(TW_LOC, "Portchan_node : Invalid node type for this action\n");
...
void Portchan_node::enqueue_packet(
  unsigned int packet_size, int port_no, int vc_no, unsigned int term_id, unsigned int app_id)
{
...
{
  // based on what was defined in the Mapper, congestion control uses second to last RNG on the
  // LP
...
{
  congestion_control_message* msg =
    (congestion_control_message*)calloc(1, sizeof(congestion_control_message));
  return msg;
}
...
int congestion_control_set_jobmap(struct codes_jobmap_ctx* ctx, int net_id)
{
  ...
  if (ctx == NULL)
  {
    // tw_printf("Congestion Control: No jobmap passed to control module - Causation Detection Not
    // Enabled")
    return -2;
  }
  else
...
void cc_router_local_controller_init(rlc_state* s, tw_lp* lp, int total_terminals, int router_id,
  int radix, int num_vcs_per_port, int* vc_sizes, double* bandwidths,
  int* workload_finished_flag_ptr)
{
  ...
  rc = configuration_get_value_double(&config, "PARAMS", "cc_single_port_decongestion_threshold",
    NULL, &p->single_port_decongestion_threshold);
  if (rc)
  {
    p->single_port_decongestion_threshold = .05;
  }

  rc = configuration_get_value_double(&config, "PARAMS", "cc_single_port_aggressor_usage_threshold",
    NULL, &p->single_port_aggressor_usage_threshold);
  if (rc)
  {
    p->single_port_aggressor_usage_threshold = .10;
  }

  rc = configuration_get_value_double(
    &config, "PARAMS", "cc_minimum_abatement_active_time", NULL, &p->minimum_abatement_time);
  if (rc)
  {
...
void cc_router_local_congestion_event(
  rlc_state* s, tw_bf* bf, congestion_control_message* msg, tw_lp* lp)
{
  switch (msg->type)
  {
    default:
...
void cc_router_local_congestion_event_rc(
  rlc_state* s, tw_bf* bf, congestion_control_message* msg, tw_lp* lp)
{
  switch (msg->type)
  {
...
void cc_router_local_congestion_event_commit(
  rlc_state* s, tw_bf* bf, congestion_control_message* msg, tw_lp* lp)
{
  switch (msg->type)
  {
...
void cc_router_received_packet(rlc_state* s, tw_lp* lp, unsigned int packet_size, int port_no,
  int vc_no, int term_id, int app_id, congestion_control_message* rc_msg)
{
  ...
  if (s->output_ports.size() > 0 && s->output_ports.count(port_no) > 0)
  {
    ...
    {
      // Then abatement for this port is already active, we need to see if this is a packet from a
      // new aggresssor regardless, we also need to extend the abatement policy

      if (s->packet_counting_tree->is_abated_terminal(term_id) == false)
      {
        int this_app_occupancy =
          s->packet_counting_tree->get_app_count_map_by_port(port_no)[app_id];
        unsigned long long port_occupancy =
          s->packet_counting_tree->get_packet_count_by_port(port_no);
        if ((this_app_occupancy / port_occupancy) >=
            s->params->single_port_aggressor_usage_threshold)
        {
          // then we haven't yet abated the terminal that send this message and we are currently in
          // congestion
          ...
          s->packet_counting_tree->mark_abated_terminal(
            port_no, term_id); // increments the abatement counter
        }
      }

      s->packet_counting_tree->set_next_possible_port_normal_time(port_no,
        cur_expire_time + bytes_to_ns(packet_size, s->router_bandwidths_on_each_port[port_no]));
    }
    else
    { // TODO NOTE: this if else only is valid if we're only monitoring ports
...
void cc_router_congestion_check(
  rlc_state* s, tw_lp* lp, int port_no, int vc_no, congestion_control_message* rc_msg)
{
  int num_rngs = 0;
  ...
  if (s->packet_counting_tree->is_port_congested(port_no) == false)
  {
    if (port_occupancy >= port_congestion_threshold_size)
    {
      ...
      tw_stime expiration_delay =
        2 * bytes_to_ns(port_congestion_threshold_size - port_decongestion_threshold_size,
              s->router_bandwidths_on_each_port[port_no]);
      // expiration_delay = max(expiration_delay, s->params->minimum_abatement_time);
      s->packet_counting_tree->set_next_possible_port_normal_time(
        port_no, tw_now(lp) + expiration_delay);

      rc_msg->to_congest = 1;
      // printf("CONGESTION DETECTED %llu\n", port_occupancy);
      set<pair<unsigned int, unsigned int>> aggressor_term_ids;

      map<unsigned int, unsigned long long> app_map =
        s->packet_counting_tree->get_app_count_map_by_port(port_no);
      ...
      for (; it != app_map.end(); it++)
      {
        ...
        {
          map<unsigned int, unsigned long long> tcm =
            s->packet_counting_tree->get_term_count_map_by_port_from_app(port_no, it->first);
      ...
      // send an abatement signal to each aggressor terminal on this port if they aren't already
      // abated
  ...
  else
  {
    if (tw_now(lp) > s->packet_counting_tree->get_next_possible_port_normal_time(port_no))
    {
      if (port_occupancy < port_decongestion_threshold_size)
      {
        ...
        rc_msg->danger_rc_deabated =
          (unsigned int*)malloc(sizeof(unsigned int) * rc_msg->size_deabated);
    ...
    //         {
    //             //if any are no longer marked abated at all, then send a normal signal
    //             tw_lpid term_lpgid = codes_mapping_get_lpid_from_relative(*it, NULL,
    //             terminal_lp_name, NULL, 0); congestion_control_message *c_msg; tw_event *e =
    //             model_net_method_congestion_event(term_lpgid, .1, s->lp, (void**)&c_msg, NULL);
    //             c_msg->type = CC_SIGNAL_NORMAL;
    //             tw_event_send(e);
    //         }
    //     }
...
void cc_router_congestion_check_rc(
  rlc_state* s, tw_lp* lp, int port_no, int vc_no, congestion_control_message* rc_msg)
{
...
static double calculate_bandwidth_usage_percent(
  int bytes_transmitted, double max_configured_bandwidth, int multiplier)
{
...
{
  double usage_percent = calculate_bandwidth_usage_percent(s->ejected_packet_bytes,
    s->params->terminal_configured_bandwidth,
    1); // multiplier for multiple rails but right now we're just using 1
  double removed_window =
    s->ejected_rate_windows[s->window_epoch % cc_bandwidth_rolling_window_count];
  ...
  s->cur_average_rate =
    (s->cur_average_rate - removed_window + usage_percent) / cc_bandwidth_rolling_window_count;
  ...
  if (*s->workloads_finished_flag_ptr == 0)
  {
    ...
    tw_event* hb_e = model_net_method_congestion_event(
      s->lp->gid, cc_bandwidth_monitoring_window, s->lp, (void**)&hb_msg, NULL);
    hb_msg->type = CC_BANDWIDTH_CHECK;
    tw_event_send(hb_e);
  }
}

void cc_terminal_process_bandwidth_check_rc(
  tlc_state* s, congestion_control_message* msg, tw_lp* lp)
{
  s->window_epoch--;
...
void cc_terminal_local_controller_init(
  tlc_state* s, tw_lp* lp, int terminal_id, int* workload_finished_flag_ptr)
{
  s->params = (cc_param*)calloc(1, sizeof(cc_param));
  cc_param* p = s->params;
  s->lp = lp;
  ...
  // configuration_get_value(&config, "PARAMS", "cc_workload_lpname", NULL, workload_lp_name,
  // MAX_NAME_LENGTH); if (strlen(workload_lp_name) <= 0) {
  ...
  //     tw_lpid work_gid = codes_mapping_get_lpid_from_relative(work_rel_id, NULL,
  //     workload_lp_name, NULL, 0); struct codes_jobmap_id job_ident =
  //     codes_jobmap_to_local_id(work_rel_id, ctx); //work rel id is job global id - TODO DOUBLE
  //     CHECK tw_lpid attached_term_id = model_net_find_local_device(DRAGONFLY_DALLY, NULL, 0,
  //     work_gid);

  //     if (attached_term_id == lp->gid && job_ident.job != -1) {
  //         s->app_id = job_ident.job; //right now there's only one app per terminal, this may
  ...
  rc = configuration_get_value_double(
    &config, "PARAMS", "cc_measurement_period", NULL, &cc_bandwidth_monitoring_window);
  if (rc)
  {
  ...
  */
  ...
  congestion_control_message* hb_msg;
  tw_event* hb_e = model_net_method_congestion_event(
    lp->gid, cc_bandwidth_monitoring_window, s->lp, (void**)&hb_msg, NULL);
  hb_msg->type = CC_BANDWIDTH_CHECK;
  tw_event_send(hb_e);
}
...
void cc_terminal_send_ack(tlc_state* s, tw_lpid original_terminal_lpgid)
{
  ...
  tw_event* ack_e = model_net_method_congestion_event(original_terminal_lpgid,
    s->params->notification_latency + .00001, s->lp, (void**)&ack_msg, NULL);
...
void cc_terminal_start_abatement(tlc_state* s, congestion_control_message* msg)
{
  ...
  // printf("%d from app%d: Abating at %.2f\n", s->terminal_id, app_id,
  // s->current_injection_bandwidth_coef);
}
...
double cc_terminal_get_current_injection_bandwidth_coef(tlc_state* s)
{
  ...
  else if (s->abatement_signal_count > 0)
  {
    ...
    // double min_injection = (1.0/codes_jobmap_get_num_ranks(s->app_id, jobmap_ctx)); //TODO
    // s->app_id is never set because it's hard for the network side to know this
    ...
    if (ret_val < 0)
      printf("%.2f = %.2f / %d\n", ret_val, s->current_injection_bandwidth_coef,
        s->abatement_signal_count);
    if (ret_val > 1)
      ret_val = 1;
    assert(ret_val <= 1);
...
void cc_terminal_local_congestion_event(
  tlc_state* s, tw_bf* bf, congestion_control_message* msg, tw_lp* lp)
{
  switch (msg->type)
  {
...
void cc_terminal_local_congestion_event_rc(
  tlc_state* s, tw_bf* bf, congestion_control_message* msg, tw_lp* lp)
{
  switch (msg->type)
  {
...
void cc_terminal_local_congestion_event_commit(
  tlc_state* s, tw_bf* bf, congestion_control_message* msg, tw_lp* lp)
{
  switch (msg->type)
  {
