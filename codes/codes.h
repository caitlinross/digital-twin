...
#define DEPRECATED
  ...
  DEPRECATED
  static inline tw_event* codes_event_new(tw_lpid dest_gid, tw_stime offset_ts, tw_lp* sender)
  {
  ...
  static inline tw_event* tw_event_new_bounded(tw_lpid dest_gid, tw_stime offset_ts, tw_lp* sender)
  {
...
#define CODES_LATENCY_RANGE (CODES_MAX_LATENCY - CODES_MIN_LATENCY)
  static inline tw_stime codes_local_latency(tw_lp* lp)
  {
    int r = g_tw_nRNG_per_lp - 1;
    tw_stime tmp;

    tmp = g_tw_lookahead + CODES_MIN_LATENCY + tw_rand_unif(&lp->rng[r]) * CODES_LATENCY_RANGE;
