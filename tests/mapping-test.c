...
typedef struct state_s
{
  ...
  char group_name[MAX_NAME_LENGTH], lp_name[MAX_NAME_LENGTH], anno[MAX_NAME_LENGTH];
} state;
...
static void init(state* ns, tw_lp* lp)
{
  ...
  codes_mapping_get_lp_info(
    lp->gid, ns->group_name, &dummy, ns->lp_name, &dummy, ns->anno, &dummy, &dummy);
  const char* anno = codes_mapping_get_annotation_by_lpid(lp->gid);
  // annotation check
  if ((anno == NULL && ns->anno[0] != '\0') || (anno != NULL && strcmp(anno, ns->anno) != 0))
  {
    fprintf(stderr,
      "LP %llu: annotations don't match: "
  ...
  tw_lpid id_from_global_rel =
    codes_mapping_get_lpid_from_relative(ns->id_global, NULL, ns->lp_name, NULL, 0);
  tw_lpid id_from_group_rel =
    codes_mapping_get_lpid_from_relative(ns->id_by_group, ns->group_name, ns->lp_name, NULL, 0);
  tw_lpid id_from_anno_rel =
    codes_mapping_get_lpid_from_relative(ns->id_by_anno, NULL, ns->lp_name, anno, 1);
  tw_lpid id_from_group_anno_rel = codes_mapping_get_lpid_from_relative(
    ns->id_by_group_anno, ns->group_name, ns->lp_name, anno, 1);
  ...
  if (lp->gid != id_from_group_rel)
  {
    fprintf(stderr,
      ...
      LLU(lp->gid), ns->lp_name, ns->group_name, ns->id_by_group, LLU(id_from_group_rel));
  }
  if (lp->gid != id_from_anno_rel)
  {
    fprintf(stderr,
      ...
      LLU(lp->gid), ns->lp_name, ns->anno, ns->id_by_group, LLU(id_from_anno_rel));
  }
  ...
  printf("TEST2 %2llu %2d %2d %2d %2d %s%s\n", LLU(lp->gid), ns->id_global, ns->id_by_group,
    ns->id_by_anno, ns->id_by_group_anno, ns->lp_name, tmp);
}
...
#define TEST_TO_NM(_expected_nm, _input, _fn_type)                                                 \
  do                                                                                               \
  {                                                                                                \
    char const* _anm = codes_mapping_get_##_fn_type##_name_by_cid(_input);                         \
    char const* _enm = (_expected_nm) == NULL ? "" : (_expected_nm);                               \
    ...
    {                                                                                              \
      fprintf(stderr,                                                                              \
        ...
        __FILE__, __LINE__, (_expected_nm) == NULL ? "null" : (_expected_nm), _anm);               \
      return 1;                                                                                    \
    }                                                                                              \
  } while (0);
...
#define TEST_TO_CID(_expected_cid, _input, _name_type, _fn_type)                                   \
  ...
  {                                                                                                \
    ...
    int _acid = codes_mapping_get_##_fn_type##_cid_by_##_name_type(_input);                        \
    if (_ecid != _acid)                                                                            \
    {                                                                                              \
      fprintf(stderr, "%s:%d: " #_name_type " name->cid check error (expected %d, got %d)\n",      \
  ...
  for (int l = 0; l < 3; l++)
  {
    for (int g = 0; g < 3; g++)
    {
      for (int a = 0; a < 3; a++)
      {
        ...
        printf("TEST1 %2d %6s %s\n", codes_mapping_get_lp_count(groups[g], 0, lps[l], annos[a], 0),
          groups[g], lpnm);
      }
      printf("TEST1 %2d %6s %s ignore annos\n",
        codes_mapping_get_lp_count(groups[g], 0, lps[l], NULL, 2), groups[g], lps[l]);
    }
  }
