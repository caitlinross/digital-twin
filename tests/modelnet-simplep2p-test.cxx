 ...
 * per second.
 *
 * It also tests the ability to add in a custom LP type that isn't built as part of codes
 */
...
void svr_register_lp_type()
{
  lp_type_register("nw-lp", &svr_lp);
}

// we'll set the network id ourselves, so that's nullptr
const bool registered =
  codes::Orchestrator::GetInstance().RegisterLPType("nw-lp", svr_register_lp_type, nullptr);
...
static void handle_ack_event(svr_state* ns, svr_msg* m, tw_lp* lp);
static void handle_req_event(svr_state* ns, svr_msg* m, tw_lp* lp);
...
const tw_optdef app_opt[] = { TWOPT_GROUP("Model net test case"),
...
static void svr_init(svr_state* ns, tw_lp* lp)
{
  ...
  m = reinterpret_cast<svr_msg*>(tw_event_data(e));
...
static void handle_kickoff_event(svr_state* ns, svr_msg* m, tw_lp* lp)
{
  ...
  m->ret = model_net_event(net_id, "test", dest_id, PAYLOAD_SZ, 0.0, sizeof(svr_msg), &m_remote,
